...
antilink_status = True  # തുടക്കത്തിൽ ഓൺ ആയിരിക്കും

# പോസ്റ്ററുകളുടെ/ലിങ്കുകളുടെ എണ്ണം ട്രാക്ക് ചെയ്യാനുള്ള ഡിക്ഷണറി
# {chat_id: {user_id: {"name": str, "count": int}}}
user_poster_counts = {}
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "/tmute [മിനിറ്റ്] - പറഞ്ഞ സമയത്തേക്ക് മ്യൂട്ട് ചെയ്യാൻ\n"
        "/unmute - മ്യൂട്ട് മാറ്റാൻ\n\n"
        "**പൊതു കമാൻഡുകൾ:**\n"
        "/leaderboard (അല്ലെങ്കിൽ /top) - Top 5 പോസ്റ്റർ ലീഡർബോർഡ് കാണാൻ\n\n"
        "**അഡ്മിൻ കമാൻഡുകൾ:**\n"
        "/antilink on - ആന്റി-ലിങ്ക് ഓൺ ചെയ്യാൻ\n"
        "/antilink off - ആന്റി-ലിങ്ക് ഓഫ് ചെയ്യാൻ\n"
...
async def show_leaderboard(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ഏറ്റവും കൂടുതൽ പോസ്റ്റർ അയച്ചവരെ അനുക്രമമായി ക്രമീകരിക്കുന്നു (Sorting)
    sorted_users = sorted(
...
async def ban_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not update.message.reply_to_message:
        await update.message.reply_text("⚠️ ഏത് യൂസറെയാണ് ബാൻ ചെയ്യേണ്ടത് അവരുടെ മെസ്സേജിന് റിപ്ലൈ ആയി ചെയ്യുക.")
        return
...
async def handle_normal_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # മെസ്സേജിൽ ലിങ്കുകൾ (URLs) ഉണ്ടോ എന്ന് പരിശോധിക്കുന്നു
    has_link = update.message.entities and any(entity.type in ["url", "text_link"] for entity in update.message.entities)

    # ലിങ്കുകൾ ഉള്ള മെസ്സേജുകൾ ആണെങ്കിൽ (അല്ലെങ്കിൽ പോസ്റ്ററുകൾ ആണെങ്കിൽ) ലീഡർബോർഡ് കൗണ്ട് കൂട്ടുന്നു
    if has_link:
        ...
        user_poster_counts[chat_id][user.id]["name"] = user.first_name # പേര് അപ്ഡേറ്റ് ചെയ്യുന്നു
        return

    # അഡ്മിൻമാർ അയക്കുന്ന ലിങ്ക് ഇല്ലാത്ത സാധാരണ മെസ്സേജുകൾ ഡിലീറ്റ് ആകരുത്
    if await is_admin(update, context):
        return

    # ലിങ്ക് അല്ലാത്ത സാധാരണ ടെക്സ്റ്റ് മെസ്സേജുകൾ ഡിലീറ്റ് ചെയ്യുന്നു
    try:
        ...
        # 3. പഴയ വാണിംഗ് മെസ്സേജ് ഡിലീറ്റ് ചെയ്യുന്നു
...
def main():
    ...
    app.add_handler(CommandHandler(["leaderboard", "top"], show_leaderboard))  # ലീഡർബോർഡ് കമാൻഡ്
    ...
    # മെമ്പർ ജോയിൻ ചെയ്യുന്നത് അറിയാൻ
    app.add_handler(MessageHandler(filters.StatusUpdate.NEW_CHAT_MEMBERS, welcome_new_member))

    # മെസ്സേജുകൾ ഫിൽട്ടർ ചെയ്യാനും കൗണ്ട് ട്രാക്ക് ചെയ്യാനും
