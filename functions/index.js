...
// ലിങ്ക് മെസ്സേജുകൾക്ക് നൽകാനുള്ള വിവിധ റിയാക്ഷനുകളുടെ ലിസ്റ്റ് (ഇമോജികൾ)
const reactions = ["👍", "🔥", "❤️", "👏", "🤩", "🎉", "⚡", "👀"];

console.log("Bot is running: 15 mins delete + Auto reactions on links enabled...");
...
🔗 ഗ്രൂപ്പിൽ ലിങ്കുകൾ വരുമ്പോൾ ഞാൻ അതിന് പല തരത്തിലുള്ള റിയാക്ഷനുകൾ നൽകുന്നതുമായിരിക്കും!
...
bot.on("message", async (msg) => {
  ...
  // ലിങ്ക് മെസ്സേജ് ആണെങ്കിൽ ഓട്ടോമാറ്റിക് റിയാക്ഷൻ ഇടുന്നു
  if (isLink) {
    try {
      // ലിസ്റ്റിൽ നിന്നും ഒരു റിയാക്ഷൻ ഇമോജി എടുക്കുന്നു
      const randomEmoji = reactions[Math.floor(Math.random() * reactions.length)];
      
      // മെസ്സേജിന് റിയാക്ഷൻ നൽകുന്നു
      await bot.setMessageReaction(chatId, msg.message_id, {
        reaction: [{ type: "emoji", emoji: randomEmoji }],
        is_big: false
      });
      console.log(`Added reaction ${randomEmoji} to link message ${msg.message_id}`);
    } catch (e) {
      console.log("Error adding reaction to link:", e.message);
    }
  }

  // അഡ്മിൻമാരുടേത് ഉൾപ്പെടെ എല്ലാ തരം മെസ്സേജുകളും 15 മിനിറ്റിന് ശേഷം ഡിലീറ്റ് ചെയ്യുന്നു
  setTimeout(async () => {
    try {
      await bot.deleteMessage(chatId, msg.message_id);
